...
const History=require('../models/PromptModel');
...
// In your getHistory function, ensure it properly filters by user_id
exports.getHistory = async (req, res) => {
  try {
    const userId = req.query.userId;
    
    if (!userId) {
      // Return session-based history for anonymous users
      const history = await History.find({ session_id: req.sessionID }).sort({ createdAt: -1 });
...
// Update prompt making Ai-Integrated Websites for Government
